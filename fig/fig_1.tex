...
\colorlet{a_meta_green}{black!50!green!20!}
\colorlet{a_meta_dark_green}{black!72!green!74!}
...
\tikzstyle{meta_data} = [block, text width=4em, fill=a_meta_green]
...
\newcommand{\InputBlock}[7]{% node1, hight, label1, node2, label2, arrow1, arrow2
	...
	\path [draw, ->, thick] (#1.east |- #1.east) -- node [above, pos=0.4,] {\tiny{#6}} (#4.west |- #4.west);
    \path [draw, ->, thick] (#4.east |- #4.east) -- node [above] {\tiny{#7}} (model.west |- #4.west);
     \path [draw, ->, thick] ($(download.south)+(0.15, 0)$) |- (#1.west);
}
...
\newcommand{\OutputBlock}[7]{% node1, hight, label1, node2, label2, arrow1, arrow2
	...
	\path [draw, ->, thick] (model.east |- #1.west) -- node [above] {\tiny{#6}} (#1.west |- #1.west);
	\path [draw, ->, thick] (#1.east |- #4.west) -- node [above] {\tiny{#7}} (#4.west |- #4.west);
}
...
\sffamily
    ...
    \node (model) [models, minimum height=5cm, minimum width=4cm, text depth=0cm] {xarray Dataset};
    		...
    		\path (model.140)+(1.3,0) node () [] {};
   		 	\path[draw=black, thick] (0.5,-1) -- (0.5,-2) -- (1.5,-2);
    		\path[draw=black, thick] (0.5,-2) -- (1,-1.5);
    		...
    		\path (model.150)+(1.2,0.4) node (dataarray) [sub_models] {DataArray};
    		\path (model.150)+(1.2,-2.8) node (meta_data) [meta_data] {meta data};
    		\path (model.150)+(3,0.5) node (coords) [sub_coords] {\small{coords}};
    

    \path ($(model)+(-10.5,5)$) node (download) [function] {Download};
	\path ($(download.north)+(0,+0.6)$) node (acc) {Accessory methods};

    \path ($(model)+(8,5)$) node (staal) [function] {Staal et al };
	\path ($(model)+(8,4)$) node (morse) [function] {Morse et al};
   ...
   \OutputBlock{grid_out_proc}{-2}{xarray}{grid_out}{Grid export}{xarray}{netCDF}
    ...
    \path [draw, <->, thick] (model) -- node {} (model.south |- attributes.north);

    \path (model)+(0,3.4) node (python) [models, minimum height=2em] {Python};    
    \path [draw, <->, thick] (model) -- node {} (model.north |- python.south);
   
   \path (model)+(0,4.6) node (python_api) [models, minimum height=2em] {Python api};    
	\path [draw, <->, thick] (python_api) -- node {} (python_api.south |- python.north);
    ...
    \path [draw, <->, thick] ($(model.east)+(0,2.1)$) --++(0:0.8)|- (staal.west);
    \path [draw, ->, thick] ($(model.east)+(0,1.8)$) --++(0:1.2)|- (morse.west);
    \path [draw, ->, thick] (morse.south) -- node {} (png.north);
    
    %Meta data
    \draw[->,a_meta_dark_green, thick] ($(download.south)+(-0.15,0)$) 
    -- ($(download.south |- meta_data.south)+(-0.15,-0.6)$) 
    -- ($(meta_data.south)+(0,-0.6)$)
    -- (meta_data.south);

     \draw[->,a_meta_dark_green, thick] (meta_data.north) -- (dataarray.south); 
